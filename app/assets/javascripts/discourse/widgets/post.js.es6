...
import { transformBasicPost } from 'discourse/widgets/post-stream';
...
createWidget('post-contents', {
  ...
  html(attrs) {
    return new RawHtml({html: `<div class='cooked'>${attrs.cooked}</div>`});
  }
});
...
createWidget('post-body', {
  ...
  html(attrs, state) {
    ...
    regular.push(this.attach('actions-summary', attrs));
