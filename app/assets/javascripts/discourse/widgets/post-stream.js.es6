...
export default createWidget('post-stream', {
  ...
  transformPost(post, prevPost, nextPost) {
  ...
  html(attrs) {
    ...
    const result = posts.toArray();
    let prevPost;
    for (let i=0; i<result.length; i++) {
      const post = result[i];
      const nextPost = (i < result.length - 1) ? result[i+i] : null;
      result[i] = this.attach('post', this.transformPost(post, prevPost, nextPost), { model: post });
      prevPost = post;
    }
    return result;
  }
